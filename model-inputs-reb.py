...
player_data = pd.read_csv("2025_reb_projs.csv")
player_cols = ['pid', 'player', 'team', 'weighted_trb', 'kalman_trb']
...
final_columns = ['player', 'team', 'weighted_trb', 'kalman_trb', 
...
final_output = final_output[final_output['weighted_trb'] != 0]
final_output = final_output[final_output['kalman_trb'] != 0]
#Drop duplicate rows


# Optionally, save to a CSV for later use
final_output.to_csv("model_inputs_trb.csv", index=False)
