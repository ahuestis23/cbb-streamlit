...
df['weight_ast'] = compute_decay_weight(df['days_since'], beta_points)

# To compute a weighted average points projection per player (for example):
weighted_points = (
  df.groupby('pid')
    .apply(lambda x: np.sum(x['AST'] * x['weight_ast']) / np.sum(x['weight_ast']))
    .reset_index(name='weighted_ast')
)
...
kalman_results_pts = df_sorted.groupby('pid')['AST'].apply(apply_kalman_filter)

kalman_projections_pts = kalman_results_pts.apply(lambda arr: arr[-1]).reset_index(name='kalman_ast')
...
df_with_proj.to_csv('2025_ast_projs.csv', index=False)

# Example feature list. Update these to match the actual column names in your data.
features = ['weighted_ast', 'kalman_ast', 'team_proj',  'team_oe', 'team_de','opp_proj', 'opp_oe', 'opp_de'] 
target = 'AST'
...
import joblib
joblib.dump(gbr_model, 'gbr_model_ast.pkl')
