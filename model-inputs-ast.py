...
player_data = pd.read_csv("2025_ast_projs.csv")
player_cols = ['pid', 'player', 'team', 'weighted_ast', 'kalman_ast']
...
final_columns = ['player', 'team', 'weighted_ast', 'kalman_ast', 
...
final_output = final_output[final_output['weighted_ast'] != 0]
final_output = final_output[final_output['kalman_ast'] != 0]
#Drop duplicate rows


# Optionally, save to a CSV for later use
final_output.to_csv("model_inputs_ast.csv", index=False)
